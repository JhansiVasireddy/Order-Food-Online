            ...
            try {
                const response = await fetch(
                    import.meta.env.VITE_BASE_URL +
                        `dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=${lat}&lng=${lng}&restaurantId=${resId}`
                );
                if (!response.ok) {
                    ...
                    const ResMenu = json?.data?.cards?.find((card) =>
                        card?.groupedCard?.cardGroupMap?.REGULAR?.cards?.filter((menu) => menu?.card?.card["@type"]?.includes("food.v2.ItemCategory"))
                    );
